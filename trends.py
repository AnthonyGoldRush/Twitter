...
def extract_words(text):
    ...
    while i<len(string):
        if string[i] not in ascii_letters and temp!='':
            rec.append(temp)
            temp = ""
...
def analyze_tweet_sentiment(tweet):
    ...
    for word in words:
        ...
        if has_sentiment(s):
            count =count + 1
            sum_s = sum_s + sentiment_value(s)
        
    if count == 0:
        return make_sentiment(None)
    return make_sentiment(sum_s/count)
...
def find_centroid(polygon):
    ...
    while i<len(polygon)-1:
        Area = Area + (latitude(polygon[i])*longitude(polygon[i+1]) - longitude(polygon[i])*latitude(polygon[i+1]))
        i+=1
...
def find_state_center(polygons):
    ...
    for x in polygons:
        tup = find_centroid(x)
        sum_x_num = sum_x_num + (tup[0]*tup[2])
        sum_y_num = sum_y_num + (tup[1]*tup[2])
        sum_denom = sum_denom + tup[2]

    return make_position(sum_x_num/sum_denom, sum_y_num/sum_denom)
...
def group_tweets_by_state(tweets):
    ...
    for i in tweets:
        ...
        for x in state_centers:
            temp_dis = geo_distance(tweet_location(i), state_centers[x])
            if temp_dis<dis:
                dis = temp_dis
                state = x

        tweets_by_state[state].append(i)
...
def average_sentiments(tweets_by_state):
    ...
    for x in tweets_by_state:
        ...
        sums = 0
        total = 0
        for i in tweets_by_state[x]:
            if has_sentiment(analyze_tweet_sentiment(i)):
                sums = sums + sentiment_value(analyze_tweet_sentiment(i))
                total+=1

        if total!=0:
            average = sums/total
            averaged_state_sentiments[x] = average
